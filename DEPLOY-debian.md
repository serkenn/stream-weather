...
| 項目 | 1080p配信（既定・CPU） | 4K配信 | 8K配信 |
|------|------------------------|--------|--------|
| CPU | 物理4コア以上推奨 | 物理8コア以上＋できればGPU | GPU必須 |
| GPU | 不要（libx264でOK） | 推奨 | **NVIDIA(NVENC)必須** |
| メモリ | 2GB以上 | 4GB以上 | 8GB以上 |
| 上り帯域 | 実測6Mbps以上 | 実測20Mbps以上 | 実測45Mbps以上 |

> **GPU非搭載なら 1080p（既定）が現実的な上限**です。libx264で1080p30は4コア程度から安定します。
> コマ落ちするときは `.env` で `X264_PRESET=ultrafast`、`FPS=24`、`VIDEO_BITRATE` を下げて調整。
> CPUの空き具合は `docker stats` で確認できます（CPU%が常時100%超なら設定を軽くする）。
...
ダッシュボードはローカル限定（127.0.0.1:18080）で公開しています。手元PCから見るにはSSHトンネル:

```bash
ssh -L 18080:127.0.0.1:18080 user@your-server
# → 手元ブラウザで http://localhost:18080
```

> ホスト側ポートは `.env` の `DASHBOARD_PORT` で変更可。`port is already allocated` が出る場合は
> 既に使用中なので別番号にするか、`docker-compose.yml` の `ports:` 行を削除（配信自体には不要）。
